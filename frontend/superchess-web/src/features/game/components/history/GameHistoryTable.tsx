...
function ResultStack({ game }: { game: GameHistoryResponse }) {
  ...
  const playerWon = game.winnerColor === game.playerColor;
  const opponentWon = game.winnerColor !== null && game.winnerColor !== game.playerColor;

  const whiteClassName = game.winnerColor === "white"
    ? playerWon
      ? "text-accent"
      : opponentWon
        ? "text-[#fa412d]"
        : "text-text-muted"
    : "text-text-muted";
  const blackClassName = game.winnerColor === "black"
    ? playerWon
      ? "text-accent"
      : opponentWon
        ? "text-[#fa412d]"
  ...
  return (
    <div className="grid justify-start gap-0.5 font-mono text-sm font-bold tabular-nums">
      <span className={whiteClassName}>{whiteScore}</span>
      <span className={blackClassName}>{blackScore}</span>
    </div>
  );
}
  ...
  return (
    <>
      {Array.from({ length: 3 }).map((_, index) => (
        <div
          key={index}
          className="grid grid-cols-[48px_minmax(0,1fr)_48px] items-center gap-3 px-4 py-4 min-[500px]:grid-cols-[48px_minmax(0,1fr)_48px_86px] md:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_112px] lg:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_70px_112px]"
        >
          ...
          <div className="hidden h-3 w-8 rounded-full bg-bg-muted md:block" />
          <div className="hidden h-3 w-16 rounded-full bg-bg-muted lg:block" />
          <div className="hidden h-3 w-12 rounded-full bg-bg-muted min-[500px]:block" />
        </div>
      ))}
    </>
  );
...
export function GameHistoryTable({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border-light bg-card-muted">
      <div className="grid grid-cols-[48px_minmax(0,1fr)_48px] items-center gap-3 border-b border-border-light bg-card-header px-4 py-3 text-[11px] font-bold uppercase tracking-[0.14em] text-text-muted min-[500px]:grid-cols-[48px_minmax(0,1fr)_48px_86px] md:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_112px] lg:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_70px_112px]">
        <span />
        <span>Players</span>
        <span>Result</span>
        <span className="hidden md:block">Accuracy</span>
        <span className="hidden lg:block">Moves</span>
        <span className="hidden min-[500px]:block">Date</span>
      </div>
      ...
      <div className="divide-y divide-border-light">
        ...
        {!isLoading && !error && games.length === 0 ? (
          <div className="grid grid-cols-[48px_minmax(0,1fr)_48px] items-center gap-3 px-4 py-4 text-sm min-[500px]:grid-cols-[48px_minmax(0,1fr)_48px_86px] md:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_112px] lg:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_70px_112px]">
            <span />
            <div>
            ...
            <span className="hidden text-text-subtle min-[500px]:block">-</span>
            <span className="hidden text-text-subtle md:block">-</span>
            <span className="hidden text-text-subtle lg:block">-</span>
        ...
        {!isLoading && !error
          ? visibleGames.map((game) => (
              <Link
                key={game.id}
                href={`/game/${game.id}`}
                className="grid grid-cols-[48px_minmax(0,1fr)_48px] items-center gap-3 px-4 py-4 text-sm transition hover:bg-bg-light min-[500px]:grid-cols-[48px_minmax(0,1fr)_48px_86px] md:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_112px] lg:grid-cols-[64px_minmax(0,1.5fr)_72px_104px_70px_112px]"
              >
                ...
                <span className="hidden text-text-muted lg:block">{game.moveCount}</span>
                <span className="hidden text-xs text-text-muted min-[500px]:block sm:text-sm">
